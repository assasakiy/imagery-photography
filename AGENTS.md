...
## 13. Sesi Terbaru (2026-08-20) — Deploy Produksi cPanel (imagery.assasakiymedia.id)
- **Konfigurasi produksi di hosting cPanel**: generate APP_KEY, `APP_DEBUG=false`, `APP_ENV=production`, `APP_URL=https://imagery.assasakiymedia.id`, `APP_LOCALE=id`. DB cPanel `assasaki_imagery` @ 127.0.0.1 (MariaDB 11.4), session/cache/queue database.
- **Migrasi + seed lengkap**: 5 squash migration (status 5/5 Ran), seeder RBAC (owner/admin/client), 35 portfolio, 15 paket, 10 blog, booking. Owner `owner@imagery.my.id`/`owner123`.
- **Media/storage**: `storage:link` dibuat, 35 cover diimpor dari WordPress via `media:import-covers`, watermark 35 foto via `media:watermark --fresh` (0 gagal). PHP limits naik (upload 64M, post 70M, memory 512M, exec 300s) via `public/.htaccess` (dipakai `lsapi_module`/`php8_module`; `.user.ini` root TIDAK terbaca untuk file di `public/`).
- **Deploy cPanel (file hosting minimal, di-commit)**: `.htaccess` root me-rewrite semua request ke `public/` + blokir file sensitif (`.env`, `vendor/`, `config/`, `composer.json`, `artisan`, dll → 403), `.user.ini`, `php.ini`. Docroot domain boleh menunjuk ke root repo (bukan harus `public/`). README ditambah bagian "Shared Hosting / cPanel".
- **Cron cPanel**: `artisan schedule:run` + `artisan queue:work --stop-when-empty --tries=3 --timeout=90` tiap menit (PHP CLI `/usr/local/bin/php`).
- **Verifikasi end-to-end**: `/`, `/up`, `/login`, `/gallery`, `/services`, `/blog`, `/contact`, `/tentang` semua 200; `/dashboard` 302 (belum login); login owner via API sukses dengan seluruh permission; file sensitif 403; aset build & storage terlayani. `composer test`: 6 passed, 2 failure = lingkungan SQLite :memory: (bukan fitur).
- **Git**: commit `4edb316` (7 file) di-rebase di atas `3445ddc` (yang sudah memperkaya `.env.example` ke placeholder produksi — versi remote dipakai, perubahan lokal pada `.env.example` dibatalkan). Remote diubah ke HTTPS untuk push; push pakai GitHub fine-grained PAT valid. Setelah rebase `git rebase --continue` wajib pakai `GIT_EDITOR=true` agar non-interaktif.
