...
- **Media bottom sheet diselaraskan dgn notif publik**: styling dicocokkan dengan `data-notif-sheet` di `layouts/app.blade.php` — backdrop `bg-zinc-950/40 backdrop-blur-sm`, sheet `max-h-[80vh] rounded-t-2xl border border-b-0 bg-white shadow-xl shadow-black/5 dark:bg-zinc-900`, header `px-4 py-3` (title `text-sm font-bold` + X `p-1`), list item `flex items-center gap-3 border-b border-line px-4 py-3 hover:bg-surface-muted` dalam `max-h-[60vh] overflow-y-auto`. Tombol "Batal" DIBUANG (konsisten: tutup via X/backdrop seperti notif).
