...
- **Pengaturan: metode login pakai Toggle + deskripsi (terbaru)**: di tab Keamanan → kartu "Keamanan Login", bagian **Metode Login** diubah dari checkbox jadi **Toggle** (`size=sm`) + deskripsi per metode (Password: email/WA+password; OTP: kode via WhatsApp/Email; Google: Login Sosial; Access Link: tautan sekali pakai). Tetap difilter: password & token selalu; otp hanya jika email/WA configured; google hanya jika login sosial aktif.
