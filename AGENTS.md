...
## 13. Sesi Terbaru (2026-08-06)
- **Normalisasi besar — `clients` digabung ke `users` (terbaru)**: tabel `clients` DROP. **`users` = akun murni**: `username` (login), `email`, `phone`, `phone_verified_at`, `password` nullable, `status` pending|active|disabled, `activated_at`, SoftDeletes + who. Kolom legacy `name` → `name_legacy` lalu dihapus, `bio/avatar_url/cover_url/social_*/role` dihapus (role via Spatie). **`user_profiles`** (`full_name/avatar/cover/bio/company/occupation/website/birth_date/gender`, 1:1) + **`social_platforms`** master (`facebook/instagram/tiktok/youtube/github/whatsapp`) + **`user_socials`** (user↔platform, `url`, `is_public`). **`projects.client_id` → `user_id`**; `client_access_tokens` drop `client_id` (tetap `user_id`). Model `Client.php` dihapus.
- **User accessor**: `name` = `profile.full_name ?? @username`; `bio/company/occupation/website` dari profile; `avatar()/cover()` via profile; `primaryRole()` spatie. `Project::client()` = alias ke `user` (bukan relasi JSON — payload memakai key `user`); `clientName()`. **Frontend pakai `project.user.*`, form pakai `user_id`.**
- **`ClientRegistrationService` berbasis User**: `ensureUser(data, role)`, `registerWithInvite(data, role, hours, actor)`, `issueInvite(User)`, `activate(User, password, ?username, ?fullName)`. Login `resolveUser` by username/email/phone. `setPassword` terima `username`+`full_name` → `activate`.
- **ClientController = role client**: `apiResource('clients',...)` parameter `{user}`; store/update terima `full_name/username/company/occupation/website` → profile; serialize `{id, username, name, email, phone, status, company, occupation, website, bio, avatar, projects_count}`. TeamController admin invite + username/status.
- **ProfileController**: payload `{username, name, bio, company, occupation, website, avatar, cover, social_*, socials[]}`; `update` pecah ke `user_profiles` (full_name/bio/company/dll + avatar/cover), `users` (username/email/phone/password/notif), dan `user_socials` (map `social_facebook→facebook` dsb, buat platform otomatis).
- **Gotcha**: relasi `UserSocial::platform()` WAJIB FK eksplisit `belongsTo(SocialPlatform::class, 'social_platform_id')` (kalau tak, infer `platform_id` → null). `TeamMember::photo_display_url` pakai `user->avatar()`.
- **Tahap 2 — Status akun + Invite + Soft Delete + Recycle Bin (arsip)**: Booking tetap lead (tanpa auto-register); akun client dibuat saat project dibuat/oleh admin. **`users` + `status`** (`pending|active|disabled`, default pending) + `activated_at` + **SoftDeletes** (`deleted_at`, `deleted_by_id`, `deleted_by_name`, `delete_reason`). **`client_access_tokens`** + `status` (`pending|expired|accepted|cancelled`) + `expires_hours` (override invite). **`users.password` jadi nullable** (akun pending tanpa password).
- **`ClientRegistrationService`** (baru, pusat pembuatan client): `registerWithInvite`, `issueInvite` (invalidasi invite lama → token baru, kirim link), `activate` (set password → active+activated_at, invite accepted). Dipakai `ClientController@store` & `ProjectController@store` (client baru → invite; aktif → tanpa invite). `AuthController@setPassword` → `activate`.
- **Invite expiry**: `RuntimeSettings::inviteExpiryHours()` (default 24, opsi 6/12/24/48/72) di tab Keamanan; override per-kirim via `expires_hours`.
- **Command `auth:process-invites`** (cron daily 03:00): invite expired → status `expired`; user pending tanpa invite valid → `disabled` (**bukan hapus**).
- **Rate limit 2 lapis** (AppServiceProvider RateLimiter): `booking` = 5/ip/jam + 3/email/hari; `forgot` = 5/email/jam + 10/ip/hari.
- **`SoftDeletesWithWho` trait** (`app/Support/`): `softDeleteBy(?reason)` — update `deleted_by_id/name` (snapshot Auth::user) + `delete_reason`, lalu delete. **PENTING**: set attr manual lalu `delete()` tak menyimpan attr lain — harus `update()` dulu.
- **Recycle Bin global**: menu utama `/dashboard/recycle-bin`, `RecycleBinController` (index/restore/forceDelete, kerangka multi-model; implementasi Client dulu). Admin Client actions: `disable`, `activate`, `soft-delete` (dgn alasan), `restore`, `force-delete`. Clients.jsx: badge status akun + dialog hapus (alasan opsional) + aksi status di modal kredensial + input durasi undangan.
...
- **OTP login dinamis & hapus override per-akun**: opsi "Masuk dengan OTP" tampil hanya jika setting global OTP aktif (`login_methods_global.otp`) DAN ada kanal terkonfigurasi; `APP_CONFIG.otp = {enabled, whatsapp, email}`; label dinamis (keduanya/WA/email). `sendOtp`/`verifyOtp` terima `identifier` via `resolveUser()`. Override per-akun dihapus (migration drop `users.allowed_methods`/`login_method`).
