...
- **Pengaturan: kartu integrasi accordion + toggle hanya saat terkonfigurasi (terbaru)**: kartu Email SMTP & WhatsApp di tab Integrasi dirombak: header ringkas = judul + badge **🟢 Terkonfigurasi / 🔴 Belum Dikonfigurasi** + tombol **"Konfigurasi"** (expand/collapse accordion, chevron rotate). Form config (SMTP fields / WA driver+fields) hanya muncul saat accordion terbuka. **Toggle "Aktifkan integrasi" hanya tampil setelah terkonfigurasi** (`meta.email_configured`/`whatsapp_configured`) — toggle OFF nonaktifkan penggunaan, config tetap tersimpan; toggle ON divalidasi (email host+port, WA field required driver), kosong → toast + tetap OFF. Backend guard `SettingsController::update`: notif_*_enabled=true tapi transport tak configured → dipaksa false.
