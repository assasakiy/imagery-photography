...
- **Notifikasi & Integrasi: badge status jadi tombol accordion**: di tab Notifikasi (`NotifCard`) & tab Integrasi (Email/WA), badge status (**Aktif/Nonaktif**, **Terkonfigurasi/Belum**) bisa diklik (chevron) utk membuka/menutup isi (daftar event / form config). Toggle aktif hanya tampil jika kanal terkonfigurasi; OFF nonaktifkan penggunaan (config tetap tersimpan); ON divalidasi (kosong → toast + tetap OFF). Backend guard: notif_*_enabled=true tapi transport tak configured → paksa false.
