...
- **Pengaturan: tab Keamanan 2 kartu + integrasi toggle validasi (terbaru)**: Tab Keamanan pecah jadi **"Keamanan Login"** (login attempts, lockout, remember, **Metode Login**) & **"Retensi File"** (`file_retention_days`), tombol Simpan terpisah (`TAB_FIELDS.security_login`/`security_file`). **Metode Login difilter**: `password` & `token` (Access Link, fitur internal) selalu tampil; `otp` tampil hanya jika `email_configured || whatsapp_configured`; `google` hanya jika `google_auth_enabled && google_client_id` — metode yang belum siap tidak ditampilkan sama sekali. **Integrasi Email/WA**: toggle OFF hanya nonaktifkan penggunaan, **konfigurasi (SMTP/WA) tetap tersimpan** (tidak dihapus). Toggle ON divalidasi: Email wajib `mail_host`+`mail_port`; WA wajib field `required` driver; kosong → toast "Lengkapi konfigurasi…" + switch tetap OFF (tanpa berkedip). Indikator **"Terkonfigurasi"/"Belum Dikonfigurasi"** di bawah switch, terpisah dari status Aktif/Nonaktif. **Backend guard** `SettingsController::update`: `notif_email_enabled`/`notif_wa_enabled` = true tapi transport tak configured → dipaksa false (guard wajib `app(RuntimeSettings::class)->emailConfigured()`/`whatsappConfigured()`, jangan variabel tak terdefinisi).
