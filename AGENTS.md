...
- **MediaViewModal — tata letak (terbaru)**: popup kini struktur **header/body/footer**. `Modal` (ui.jsx) punya prop baru `footer` → di fullscreen: header `shrink-0` (atas) + body `overflow-y-auto` (scroll) + footer `shrink-0 border-t` (bawah), header & footer SELALU sticky, hanya body yang scroll. Footer tombol (Info / Salin URL / Edit) dipindah ke prop `footer` (di luar area media). Tombol info (tiga titik) ada di footer (bukan overlay media). Padding body popup diminimalkan via `bodyClassName` (default `p-4 sm:p-6`; view media `p-[2px]`, gap `[2px]`). Overlay info tetap menutupi area media saat toggle. Label tombol footer: **desktop ikon+nama, mobile ikon saja** (span `hidden sm:inline`).
