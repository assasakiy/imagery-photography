...
- **Upload foto/video final via POPUP modal** (ProjectDetail editing): tombol "Unggah Foto & Video" buka `Modal` berisi bagian Foto (staged `photoQueue` banyak file + tombol Unggah) & Video (pasangan preview/original) + **bar progress upload** (`uploadProgress` %, dipakai foto & video via `onUploadProgress`). `submitPhotos` (ganti `uploadPhotos`) & `uploadVideo` kini bertahap. Aset tetap tak tampil di halaman proyek — cek di halaman Preview.
