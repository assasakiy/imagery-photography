...
- **Pengaturan: Metode Login cek AKTIF bukan configured (terbaru)**: filter bagian **Metode Login** berubah dari `email_configured/whatsapp_configured` → **`email_enabled/whatsapp_enabled`** (`channelEnabled`). Metode OTP hanya tampil bila integrasi WA/Email **diaktifkan** (aktif = sudah configured via guard). Google tetap `google_auth_enabled && google_client_id`. Konsisten: `APP_CONFIG.otp` di `app.blade.php` pakai `channelEnabled('whatsapp')`/`('email')` (bukan configured) → opsi "Masuk dengan OTP" di login hanya muncul saat kanal aktif. Utk UI: Metode Login pakai Toggle + deskripsi per metode.
- **Pengaturan: kartu integrasi accordion + toggle hanya saat terkonfigurasi**: kartu Email SMTP & WhatsApp: header ringkas + badge **🟢 Terkonfigurasi / 🔴 Belum** + tombol **"Konfigurasi"** (accordion). Form config hanya saat terbuka. Toggle aktif hanya setelah terkonfigurasi; OFF nonaktifkan penggunaan (config tetap tersimpan); ON divalidasi (kosong → toast + tetap OFF). Backend guard: notif_*_enabled=true tapi transport tak configured → paksa false.
