...
- **Dashboard Admin Analitik**: Halaman Dashboard admin diperkaya ala dashboard analitik umum — baris KPI, baris statistik sekunder (selesai/menunggu/belum dibaca/portofolio), **panel pendapatan** dengan strip KPI (bulan ini, bulan lalu, rata-rata/proyek, menunggu bayar) + badge pertumbuhan % + **grafik batang 6 bulan** (CSS murni, tanpa library), **distribusi status proyek** (progress bar), **Tautan Cepat** (panel terpisah — BUKAN di header/title), dan 3 panel terbaru (pembayaran/proyek/pesan). Backend `DashboardController::stats` ditambah `revenue_this_month`, `revenue_last_month`, `pending_amount`, `avg_per_project`, `revenue_by_month`, `status_breakdown`.
