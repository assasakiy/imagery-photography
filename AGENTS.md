...
- **OTP: kanal cek AKTIF di seluruh lapisan (terbaru)**: `NotificationService::otpChannel()` berubah dari `emailConfigured()`/`whatsappConfigured()` → **`channelAvailable()`** (= configured && enabled). OTP hanya dikirim via kanal yang benar-benar diaktifkan; konsisten dgn halaman login (`APP_CONFIG.otp` pakai `channelEnabled`) & tab Keamanan Metode Login (`meta.email_enabled`/`whatsapp_enabled`). Metode Login difilter by aktif, bukan configured.
