...
- **MediaViewModal — tata letak (terbaru)**: tombol info (tiga titik) dipindah dari overlay kanan-atas media ke **footer popup** (baris tombol: Info / Salin URL / Edit, `px-2.5 py-1.5`). Footer diperkecil (`px-3 py-2` + border-t). Padding body popup diminimalkan lewat prop baru `Modal bodyClassName` (default `p-4 sm:p-6`; view media pakai `p-[2px]`, gap `[2px]`). Overlay info tetap menutupi area media saat toggle.
