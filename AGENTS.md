...
- **"Populer" otomatis**: layanan (Package) kini dihitung dari jumlah booking berstatus `confirmed`/`converted`, BUKAN flag manual `is_popular`. `Package::withBookingCount()` scope (`bookings as booking_count`). Pemakaian: LandingPageController mode popular, ContactController `popularPackages`, BookingController sidebar, badge Populer di `home.blade.php`/`package-card.blade.php` (booking_count > 0), `PackageController` serialize `booking_count`. Kolom `is_popular` di tabel `packages` **DROPPED** (migrasi `2026_08_20_000001_drop_is_popular_from_packages.php`), dihapus dari model/API/seeder/view/JS. `is_featured` (Unggulan) tetap manual (anjuran admin). Dashboard `Services.jsx`: checkbox Populer diganti info jumlah booking read-only. Blog populer sudah otomatis (`views_count` desc).
