...
## 13. Sesi Terbaru (2026-08-20) — Analytics pengunjung + Cookie Consent (self-hosted)
- **Analytics self-hosted** (tanpa pihak ketiga): tracking kunjungan (`page_views`), agregasi harian (`page_view_daily`), riwayat consent (`cookie_consents`). Migrasi: `database/migrations/2026_08_20_000000_create_analytics_tables.php`. Dokumentasi lengkap: `docs/analytics_and_cookie_consent.md`.
- **Cookie consent UU PDP**: banner 3 opsi (Terima Semua / Tolak / Kustom) di `resources/views/partials/cookie-consent.blade.php` + logika di `resources/js/app.js`. Tracking HANYA aktif jika cookie `cookie_consent=all`; IP di-hash SHA-256+APP_KEY (tak disimpan mentah). `POST /api/analytics/consent` CSRF-exempt, rate-limit `analytics.consent` 20/jam/IP.
...
- **Layanan/Controller**: `VisitTracker` (record + parse UA + rollup), `AnalyticsService` (overview/visits/accounts/behavior/raw), `AnalyticsController` (routes `/api/analytics/*`, guard `role:owner|admin`), command `analytics:process` (scheduled daily 02:30 di `routes/console.php`). Tombol Rollup manual juga ada di halaman.
- **UI dashboard**: halaman Analitik `Analytics.jsx` (route `/dashboard/analytics`, menu "Analitik", ikon baru di `Icon.jsx`: bar-chart-3, activity, laptop, user-check, shield) — stat cards, trend chart SVG 30 hari, tab Kunjungan/Akun/Perilaku, log mentah. Tab settings "Analitik" (`AnalyticsTab.jsx`) untuk toggle analytics + banner + teks cookie (accessor `analyticsEnabled()`/`cookieBannerEnabled()`/`cookieBannerMessage()` di `RuntimeSettings`).
- **Bug fix**: User model kurang relasi `loginHistories()` (ditambahkan); query analytics TIDAK boleh select kolom `name` (accessor, bukan kolom) — pilih `id, username, email`; `VisitTracker` import `PageViewDaily` hilang. Test suite membaik (7 error → 2 failure) setelah fix exception RuntimeSettings; 2 failure sisa = lingkungan test (SQLite :memory: tanpa migrasi), bukan dari fitur ini.
- **Runtime**: app jalan via `php artisan serve --host=0.0.0.0 --port=20128` (localhost IP 129.225.7.177), MySQL Coolify `10.0.2.7:3306`/`imagery_photography`.
