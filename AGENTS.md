...
- **"Populer" otomatis**: layanan (Package) kini dihitung dari jumlah booking berstatus `confirmed`/`converted`, BUKAN flag manual `is_popular`. `Package::withBookingCount()` scope (`bookings as booking_count`). Pemakaian: LandingPageController mode popular, ContactController `popularPackages`, BookingController sidebar (`is_popular = booking_count > 0`), badge Populer di `home.blade.php`/`package-card.blade.php` (booking_count > 0), `PackageController` serialize `booking_count`. Dashboard `Services.jsx`: checkbox Populer dihapus → info jumlah booking read-only. Blog populer sudah otomatis (`views_count` desc). `is_popular` kolom tetap ada tapi tak lagi menentukan populer.
...
- **Runtime**: app jalan via `php artisan serve --host=0.0.0.0 --port=20128` (IP 129.225.7.177), MySQL Coolify `10.0.2.7:3306`/`imagery_photography`. Git identity repo: `assasakiy <assasakiy@users.noreply.github.com>` (push masih butuh kredensial GitHub).
