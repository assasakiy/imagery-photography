...
- **Magic Link & Recovery + Notifikasi routing (terbaru)**: `client_access_tokens` sekarang multi-purpose: `project_id` nullable (`nullOnDelete`), kolom `purpose` (`invite`|`recovery`|`project`), `created_by_type`/`created_by_id` (nullable, utk audit), masa berlaku per purpose (`invite`=7 hari, `recovery`=30 mnt, `project`=24 jam) via `ClientAccessToken::createToken()` & `PURPOSE_LIFETIME`. `AuthController@accessViaToken` behavior by purpose: `invite`→`/set-password?token`, `recovery`→`/reset-password?token`, `project`→auto-login→dashboard. API baru: `POST /api/forgot`, `/api/set-password`, `/api/reset-password`, `/api/clients/{client}/credentials`, `/api/clients/{client}/token/{purpose}`. **OTP TETAP di mekanisme existing (tidak digabung ke token)**. Notifikasi digenerikkan: enum `App\Services\NotificationType` + `NotificationService::send(NotificationType, User, data)` — routing channel berdasar kategori (AUTH/ORDER/BILLING→WA→Email fallback; CONTENT→Email), teks WA ringkas per type, email sbg salinan utk operasional. `ProjectController@regenerateCredentials` dipertahankan utk kompatibilitas tapi UI kredensial dipindah ke **halaman Klien** (single source of truth); **ProjectDetail hanya info kontak client read-only + tombol "Lihat Detail Klien"** (QR/copy/wa share/regenerate dihapus dari proyek). Halaman SPA baru (guest route web): `/forgot`, `/set-password`, `/reset-password`. Login.jsx + link "Lupa Password?". Catatan: saat `issueToken` pastikan `user.phone` disinkron dari `client.phone` (WA butuh phone user; fallback email bila kosong).
