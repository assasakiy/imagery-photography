...
## 13. Sesi Terbaru (2026-08-20) — Aktivitas User, Subscriber, Blog Engagement (Fase 1-4)
- **Fase 1 — Presence/online (commit `1287d40`)**: kolom `users.last_seen_at` (squash 000000 + migrasi sementara utk DB lama, migrasi tetap 5 file); middleware `UpdateLastSeen` (heartbeat, throttle 60 dtk, grup `web`); `User::isOnline()/presence()/onlineThresholdSeconds()` (default 180 dtk, env `PRESENCE_ONLINE_THRESHOLD_SECONDS`); command `session:reap-stale-logins` (tutup LoginHistory nyangkut, schedule every 5 menit); `GET /api/audit/online-users` + field `online` di login-history (fix query pakai `profile.full_name`); UI `PresenceBadge.jsx` di Team/Clients + tab "Kehadiran" di AuditLog; fix crash `History.jsx` (`ACTION_LABEL[h.action]`).
- **Fase 2 — Subscribe blog (commit `92f7f6d`)**: role `subscriber`; `AuthController::subscribe()` (POST /api/subscribe, throttle `subscribe.send`) → buat user subscriber pending via `ClientRegistrationService::ensureUser` + OTP; `subscribeVerify()` (throttle `subscribe.verify`) → aktifkan + login; `register()` (throttle `auth.register`) → client+subscriber aktif + password (tanpa OTP); `createUser()` public + `createUserInternal()` private (fix duplikat assignRole dgn array_unique); `GoogleAuthController` diperluas utk semua pengguna (user baru → subscriber aktif) + setting `google_subscriber_registration`; tombol Subscribe navbar + modal `partials/subscribe-modal.blade.php` + handler JS di `app.js`.
- **Fase 3 — Bookmark publik (commit `f1e46b8`)**: state `isBookmarked` di `BlogController::show`; tombol "Simpan/Tersimpan" (`data-bookmark-toggle`) di `landing_pages/blog/show.blade.php` untuk subscriber/client, guest diarahkan ke modal subscribe; JS toggle di `app.js`.
- **Fase 4 — Like & Komentar (commit `c89adac`)**: tabel `likes` & `comments` (morph, `parent_id`, `status=approved|hidden`, `approved_at`) di squash 000004 (fresh install; DB lama via migrasi sementara lalu dihapus → tetap 5 file); trait `Likeable`/`Commentable` di `Blog`; relasi `likes()/comments()` di `User`; `EngagementController` (toggleLike, comments, storeComment, destroyComment, moderateList, moderate) — like/komentar khusus subscriber/client, moderasi owner/admin; UI like+counter, form komentar flat + list, moderasi via API; JS handlers di `app.js`.
- **Perbaikan pasca-Fase 4**: `ensureCanEngage` kini mengizinkan role `owner`/`admin` ikut like & komentar (sebelumnya hanya subscriber/client) — blade `show.blade.php` disesuaikan (`$canEngage` + kondisi section komentar). Halaman dashboard `/dashboard/comments` baru (`Comments.jsx`) untuk moderasi komentar (tab status, setujui/sembunyikan/hapus) memakai `GET /api/comments/moderate/list?status=` + `PATCH /api/comments/{comment}/moderate`; `serializeComment` kini menyertakan `status` & `target` (type/id/title) saat relation `commentable` dimuat.
- **Fix bug email `AlertMail`**: property `message` dibenturkan variabel `$message` (objek `Illuminate\Mail\Message`) yang selalu dipasok Laravel ke view mail → `htmlspecialchars` error saat kirim alert login/OTP. Property di-rename ke `body` (view `mail/alert.blade.php` memakai `{!! $body !!}`).
- **Manajemen Subscriber**: `SubscriberController` baru (index/show/destroy) — `GET /api/subscribers` (list, search, paginated), `GET /api/subscribers/{user}` (detail + counts bookmarks/likes/comments + data terakhir), `DELETE /api/subscribers/{user}` (soft delete, hanya subscriber murni — user dengan role client/owner/admin ditolak). Routes di group `role:owner|admin`. Halaman dashboard `/dashboard/subscribers` (`Subscribers.jsx`) — tabel dengan kolom nama, email, status, badge online, bookmark/suka/komentar count, join date; detail modal; hapus. Nav sidebar "Interaksi & Klien" → "Subscriber".
- **Gotcha baru**: `$authUser` hanya tersedia di layout (bukan section view) — gunakan `auth()->user()` langsung di blade; JANGAN pakai `hasPermissionTo('manage-comments')` (permission belum dibuat) — moderasi cukup cek `hasRole('owner')|('admin')`.
- **Dokumentasi**: detail lengkap ada di `docs/blog_engagement.md`. GitHub PAT disimpan via `credential.helper store` (`~/.git-credentials`, chmod 600) — push aman tanpa interaksi. Repo: `assasakiy/imagery-photography` (privat), origin/main sekarang `c89adac`.
