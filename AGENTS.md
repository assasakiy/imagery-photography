...
- **Fix: owner tak lihat project di dashboard (terbaru)** — `ProjectController::index` & `DashboardController` cek `isAdmin()` (hanya role `admin`), sehingga owner (role `owner`) tak lolos → jatuh ke cabang client → kosong. Ganti ke `isStaff()` (hasRole owner/admin). Frontend `Projects.jsx`/`ProjectDetail.jsx`/`Dashboard.jsx` juga `user.role === 'admin'` → ganti ke `['admin','owner'].includes(role)`. Gejala: project ada di DB & bisa diakses via URL `/dashboard/projects/{id}` tapi kosong di list dashboard.
