...
## 13. Sesi Terbaru — Modernisasi UI Tagihan & Alur Pembayaran Klien
- **Sinkronisasi repo upstream (`ee01377`):** 9 commit baru — alur pembayaran invoice klien pindah ke halaman khusus `/dashboard/client-invoices/{id}/bayar` (wizard: Pilih Metode → Instruksi → Konfirmasi; file PayInvoice*.jsx), PaymentModal lama dinonaktifkan, notifikasi pembayaran admin/klien, perbaikan crash & mapping metode, custom path media.
- **Review enterprise + fix bug alur bayar (`9d784fc`):** (1) dobel prefix `INV-INV-` di header PayInvoice (number DB sudah berprefix — wajib guard `startsWith('INV-')`); (2) wizard bayar bisa dibuka saat `pending_verification` → risiko bukti ganda, kini diblokir layar pemberitahuan; (3) tombol gateway tanpa loading → double-click bisa buat 2 transaksi TriPay, kini disabled+spinner via state `proceeding`; (4) dead code `<PaymentModal open={false}/>` dihapus.
- **Kartu tagihan maksimal 2 tombol sesuai status:** unpaid → [Bayar Sekarang][Detail Tagihan]; pending_verification → [Menunggu Verifikasi (disabled)][Detail Tagihan]; proof_rejected → [Upload Ulang Bukti][Detail Tagihan]; paid → [Detail Pembayaran][Lihat Pesanan]. Ikon folder terpisah dihapus. Kartu kini menampilkan jatuh tempo (merah jika lewat) dan progress bar terbayar (% + nominal).
- **InvoiceDetailModal didesain ulang gaya dokumen enterprise:** header gradient brand (nomor INV mono besar + badge status + terbit/jatuh tempo), hero nominal sisa tagihan + progress bar %, grid rincian (Total/DP/Sudah Dibayar) via komponen Stat, banner alasan penolakan dari `latest_payment.notes`, dan riwayat pembayaran berupa timeline vertikal (titik status berwarna, jumlah, metode+kanal via methodMeta, tanggal, thumbnail bukti klik-buka). Footer CTA kontekstual (Bayar Sekarang / Upload Ulang Bukti / Tutup).
- **Ikon baru** di `Icon.jsx`: `external-link`, `check-circle` (lucide path inline).
