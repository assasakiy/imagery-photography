...
## 13. Sesi Terbaru (2026-08-13) — Progressive Loading, Skeleton Loading, dan Perapian UX
- **Arsitektur Progressive Loading**: Seluruh halaman dashboard sekarang lazy-load (code-splitting) via *registry* `routes/routeImports.js` (tiap path → `import()`), dengan (1) **idle prefetch** seluruh rute lainnya 1.5 detik setelah Dashboard mount (`routes/prefetchAll.js`, satu-per-satu via `requestIdleCallback`, fallback `setTimeout`), (2) **hover/focus/touch preload** di sidebar (`Layout.jsx` memanggil `preloadRoute`), dan (3) **skip otomatis** di koneksi lambat (`saveData`/`2g`). `routes/preloadRoute.js` punya cache promise + `normalizePath` agar path berparameter (`:id`) ikut terpreload. Ukuran bundle utama turun 335 kB → 267 kB.
- **Skeleton Loading menggantikan Spinner**: Semua halaman yang sebelumnya menampilkan `<Spinner />` penuh saat fetch data kini memakai skeleton komponen `Skeleton.jsx` (varian `card`/`table`/`form` → `components/skeletons/`). **Pola wajib**: `PageHeader` (judul) SELALU dirender, skeleton hanya untuk area konten, sehingga saat data selesai **tidak ada lompatan layout**. Skeleton TIDAK boleh berisi blok judul tiruan/`py-2` ekstra. Fallback route-level `PageFallback.jsx` menunda skeleton 150ms (desktop) / 200ms (mobile) agar tidak berkedip; `RouteErrorBoundary` auto-reload bila chunk load gagal.
- **Auth tanpa spinner awal**: Halaman `Login`/`ForgotPassword`/`ResetPassword`/`SetPassword` kini *eager import* (bukan lazy) sehingga langsung tampil tanpa fallback loading; `LoginRoute` tidak lagi merender spinner.
- **Perapian kecil**: Badge status proyek/pesanan dipindah sebaris dengan nomor `PSN-{no}` di card (judul di bawahnya). Spinner kecil tetap dipertahankan untuk loading bagian dalam konten (thread chat, riwayat pembayaran).
