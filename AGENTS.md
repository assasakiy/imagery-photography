...
- **Layanan — 2 tab (terbaru)**: halaman Layanan dashboard dipecah jadi **2 tab** (style sama dgn AuditLog: `flex gap-1 overflow-x-auto rounded-2xl border-line bg-surface p-1`, tombol `flex-1`, aktif `bg-brand-600 text-white shadow`): **Layanan** (card layanan + tombol Tambah Layanan) & **Kategori Harga** (CRUD kategori/tabel + tombol Tambah Kategori). Tombol header berubah sesuai tab aktif.
- **Layanan — kategori harga & tabel dinamis**: tabel/grid harga di halaman publik `/services` (sebelumnya hard-coded di Blade) kini dikelola dari dashboard. Tabel baru `service_categories` (label, title, description, layout `table|grid`, columns JSON, order, published) & `service_items` (category_id, name, values JSON, order). Controller `ServiceCategoryController` API CRUD — `index()` with `items`; create/update terima `items` array (sync delete+recreate item). **PENTING**: method update/destroy Wajib param `ServiceCategory $serviceCategory` (camelCase) — Laravel implicit binding cocokkan nama variabel ke route param `{service_category}`; kalau `$category`, binding gagal → `service_category_id = null` → Integrity constraint saat sync items. Seeder `ServiceCategorySeeder` isi data dari Blade lama (I Satuan tabel 6 baris, II Premium tabel 3, III Ultimate grid 6). Dashboard `Services.jsx`: card layanan edit/hapus pindah ke **pojok kanan atas** (`absolute right-3 top-3`, opasitas saat hover, `opacity-0 group-hover:opacity-100`); seksi baru **"Kategori Harga & Tabel"** (CRUD kategori, layout, kolom tabel dinamis, item per baris — kolom pertama selalu nama). Blade publik loop `ServiceCategory` `where('published', true)` (tabel & grid), **Catatan Penting tetap hard-coded** (nanti dikelola dari tempat lain).
