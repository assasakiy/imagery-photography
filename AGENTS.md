...
- **Layanan — kolom `terms` (terbaru)**: "Edit + Softfile" sebelumnya salah ditaruh di `duration`. Kini `services` punya kolom baru **`terms`** (ketentuan layanan, mis. "Edit + Softfile" utk foto) dipisah dari **`duration`** (durasi nyata: "3 Jam", "1 Jam", "3-4 Menit", "1-2 Menit"). Migration `2026_08_05_210000_add_terms_to_services`; seeder & data existing diperbarui (foto → terms "Edit + Softfile"). Landing tabel/grid satuan tampilkan terms + duration sbg sub-label. Dashboard master: field form & kolom tabel "Ketentuan" + "Durasi" terpisah.
