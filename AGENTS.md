...
- **Dashboard Admin Analitik**: Halaman Dashboard admin diperkaya ala dashboard analitik umum — baris KPI, baris statistik sekunder (selesai/menunggu/belum dibaca/portofolio), **grafik pendapatan 6 bulan** (bar chart CSS murni, tanpa library chart) dan **distribusi status proyek** (progress bar). Backend `DashboardController::stats` ditambah `revenue_by_month` (grouping `paid_at` bulanan 6 bulan, hanya payment `confirmed`) dan `status_breakdown` (COUNT per status). Ikon `layers` baru ditambahkan di `components/Icon.jsx`.
