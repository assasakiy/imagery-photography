export function isVerified(user, explicit) {
...
export function VerifiedBadge({ className = '' }) {
    return (
        <span className={`inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-sky-500 text-white ${className}`} title="Akun terverifikasi" aria-label="Akun terverifikasi">
...
export default function VerifiedName({ user, name, verified, className = '' }) {
    return (
        <span className={`inline-flex min-w-0 items-center gap-1 ${className}`}>
            ...
            {isVerified(user, verified) && <VerifiedBadge />}
        </span>
    );
}
