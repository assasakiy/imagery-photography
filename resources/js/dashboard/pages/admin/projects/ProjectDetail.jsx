...
import { formatRupiah, formatDate, EmptyState } from '../../../components/ui';
...
export default function ProjectDetail() {
    ...
    const submitUpload = async () => {
        ...
        const hasVid = videoForm.preview && videoForm.original;
        if ((!hasPhotos && !hasVid) || uploading) return;

        setUploading(true);
        setUploadProgress(0);
        try {
            if (hasPhotos) await uploadPhotosBatch();
            if (hasVid) await uploadVideoPair();
            setPhotoQueue([]);
    ...
    return (
        <>
            ...
            <RerequestModal open={rerequestOpen} onClose={() => setRerequestOpen(false)} saving={saving} rerequestNote={rerequestNote} setRerequestNote={setRerequestNote} onSubmit={submitRerequest} />
