@php
    ...
    $verified = $verified ?? ($user?->hasRole(['owner', 'admin']) ?? false);
@endphp
<span class="inline-flex min-w-0 items-center gap-1 {{ $class ?? '' }}">
    <span class="truncate">{{ $displayName }}</span>
    @if ($verified)
        <span class="inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-sky-500 text-white" title="Akun terverifikasi" aria-label="Akun terverifikasi">
